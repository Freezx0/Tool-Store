...
export const PromotionsPage: React.FC = () => {
  const { products, addToCart, isLoading, t } = useStore();
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <div className="bg-gradient-to-r from-red-600 to-orange-600 rounded-3xl p-8 md:p-12 mb-12 text-white relative overflow-hidden">
        <div className="relative z-10 max-w-2xl">
          <div className="flex items-center gap-2 bg-white/20 backdrop-blur-sm w-fit px-3 py-1 rounded-full mb-4">
            ...
            <span className="text-sm font-bold uppercase tracking-wide">{t('promotions.title')}</span>
          </div>
          <h1 className="text-4xl md:text-5xl font-extrabold mb-4 leading-tight">{t('promotions.subtitle')}</h1>
      ...
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{t('promotions.discounted')}</h2>
        <span className="text-slate-500">{discountedProducts.length} {t('catalog.found')}</span>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          <p className="text-xl text-slate-500 mb-4">{t('promotions.noPromos')}</p>
          <Link to="/catalog">
            <Button>{t('promotions.goToCatalog')}</Button>
          </Link>
        </div>
      )}
    </div>
